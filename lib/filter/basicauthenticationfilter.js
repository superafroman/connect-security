/**
 * Authentication filter that handles Basic authentication requests.
 */
...
var BasicAuthenticationFilter = module.exports = function(options) {
  ...
  if (!this.ignoreFailure && !this.entryPoint) {
    throw new Error(
        'EntryPoint must be provided if failures are not to be ignored.');
  }
...
BasicAuthenticationFilter.prototype.success = function(request, response,
    callback) {
...
BasicAuthenticationFilter.prototype.failure = function(request, response,
    callback) {
