...
var UrlInterceptFilter = require('./filter/urlinterceptfilter');
...
var Security = module.exports = function(options) {
  ...
  // FIXME: displaces remembermeauthenticationfilter
  if (options.interceptUrls) {
    filters.unshift(new UrlInterceptFilter(options.interceptUrls));
  }
...
Security.preAuthorize = function(access, request, callback) {
  if (ElResolver.resolve(access, request)) {
    callback();
  } else {
